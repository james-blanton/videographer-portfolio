...
If the user clicks on the "MOTION GRAPHICS" tab while on video.php, then this file will be displayed on the page.
This particular tab will display all of the videos that have been entered in to the database with the category of "Motion Graphics"
...
// Select statement to get all of the data on every editorial video in the database that does not have a a boolean flag for display_hide set to 1
$sql= "SELECT id, description, filename, fileextension, display_hide FROM video WHERE category = 'Motion Graphics' AND display_hide = 0 ORDER BY id";
...
while($row = $result->fetch_assoc()) {
	...
	$id = $row['id']; // the unique id / primary key for the commercial video
